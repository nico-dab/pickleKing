...
const Leaderboard = ({ players, onUpdateStats }) => {
  // Sort players by points (descending), then by win percentage
  ...
  const updatePlayerPoints = (playerId, change) => {
    ...
    const newPoints = Math.max(0, player.points + change);
    onUpdateStats(playerId, player.wins, player.losses, newPoints);
  };
